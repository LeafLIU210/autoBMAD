...
async def test_execute_with_custom_dirs():
    ...
    async with anyio.create_task_group() as tg:
        ...
        # 直接 mock agents 的 execute 方法以验证参数传递
        with patch.object(controller.ruff_agent, 'execute', new_callable=AsyncMock) as mock_ruff, \
             patch.object(controller.pyright_agent, 'execute', new_callable=AsyncMock) as mock_pyright, \
             patch.object(controller.pytest_agent, 'execute', new_callable=AsyncMock) as mock_pytest:
            mock_ruff.return_value = {"status": "completed", "errors": 0, "warnings": 0, "files_checked": 10, "message": "OK"}
            mock_pyright.return_value = {"status": "completed", "errors": 0, "warnings": 0, "files_checked": 10, "message": "OK"}
            mock_pytest.return_value = {"status": "completed", "tests_passed": 10, "tests_failed": 0, "tests_errors": 0, "coverage": 90.0, "total_tests": 10, "message": "OK"}
            ...
            assert mock_ruff.call_args.kwargs.get("source_dir") == "/custom/src"
            mock_pyright.assert_called_once()
            assert mock_pyright.call_args.kwargs.get("source_dir") == "/custom/src"
            mock_pytest.assert_called_once()
            assert mock_pytest.call_args.kwargs.get("test_dir") == "/custom/tests"
