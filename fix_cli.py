# Fix the cli.py file
content = '''"""Command-Line Interface for Bubble Sort Algorithm."""
...
def parse_array_input(array_str: str) -> List[Union[int, float]]:
...
def format_output(
    ...
    if format_type == "json":
        ...
        if show_stats:
            steps = get_sorting_steps(data)
            output_lines.append(f"Comparisons: {len(steps) * len(data) if len(steps) > 1 else 0}")
...
def main():
  ...
  %(prog)s --format json [5, 3, 8, 1]"""
    ...
    input_group.add_argument(
        "array",
        nargs="?",
        help="Array to sort (e.g., \"[1, 3, 2]\" or \"1, 3, 2\")"
    )
    ...
    input_group.add_argument("--batch", action="store_true", help="Run in batch mode (process multiple files)")
    ...
    parser.add_argument("--stats", action="store_true", help="Include statistics in output")
