...
class TestDevQaController:
    ...
    async def test_make_decision_draft_state(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # 模拟状态解析返回不同值
            call_count = [0]
            def mock_state_agent(*args, **kwargs):
                call_count[0] += 1
                if call_count[0] == 1:
                    return "Draft"  # 第一次调用返回Draft
                else:
                    return "Ready for Review"  # 第二次调用返回Ready for Review

            # 模拟Dev Agent执行
            with patch.object(controller.state_agent, 'execute', side_effect=mock_state_agent):
                with patch.object(controller.dev_agent, 'execute', return_value=True) as mock_dev:
                    # 测试决策
                    result = await controller._make_decision("Start")

                    # 验证结果（应该返回Ready for Review，表示完成开发）
    ...
    async def test_failed_state_within_max_rounds(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # 模拟状态解析一直返回Failed（模拟无法修复的情况）
            with patch.object(controller.state_agent, 'execute', return_value="Failed"):
