...
使用方式：
    ...
    python scripts/install_post_commit_hook.py --verify-only
"""
...
class PathResolver:
    ...
    def __init__(self, project_root: Path, venv_name: str = "venv", verbose: bool = False):
...
class PostCommitInstaller:
    ...
    def __init__(self, project_root: Path | None = None,
        ...
        self.update_script: Path = self.scripts_dir / "update_claude_md.py"
        self.log_file: Path = self.scripts_dir / "post-commit.log"
    ...
    def check_dependencies(self) -> bool:
        """检查必要依赖"""
        Logger.step(1.5, "检查依赖")

        # 检查 anthropic 包
        try:
            import anthropic
            Logger.success("anthropic 包已安装")
        except ImportError:
            Logger.warning("anthropic 包未安装，AI 分析功能不可用")
            Logger.step(1.6, "如需安装请运行: pip install anthropic")

        return True

    def check_update_script(self) -> bool:
        """检查更新脚本"""
        Logger.step(3.5, "检查更新脚本")

        if not self.update_script.exists():
            Logger.error(f"更新脚本不存在: {self.update_script}")
            Logger.warning("CLAUDE.md 自动更新功能将不可用")
            return False

        Logger.success(f"更新脚本存在: {self.update_script}")
        return True
    ...
    def _generate_hook_content(self) -> str:
        """Generate hook script content (Bash format for cross-platform compatibility)"""
        python_path = self.venv_python
        script_path = self.post_commit_source

        # Use Unix-style shebang for cross-platform compatibility
        # This works on Git Bash, WSL, Linux, macOS
        return f"""#!/bin/bash
# Git post-commit hook - auto-update CLAUDE.md
# Generated by install_post_commit_hook.py
...
"""
    ...
    def run(self) -> bool:
        """运行安装流程"""
        Logger.section(f"Python post-commit Hook 安装程序 v{self.VERSION}")
        ...
        steps = [
            ...
            ("检查更新脚本", self.check_update_script),
        ...
        print("  4. 实时日志写入 scripts/post-commit.log")
        print()
        print("常用命令：")
        print(f"  手动触发更新: {self.venv_python} {self.scripts_dir / 'update_claude_md.py'}")
        print(f"  查看日志: type scripts\\post-commit.log  (Windows)")
        print(f"  实时查看日志: Get-Content scripts\\post-commit.log -Wait  (Windows)")
        print()
