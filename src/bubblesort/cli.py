...
class ArgparseArgs(TypedDict):
    """Type definition for argparse.Namespace."""
...
def validate_data(data: list[int | float]) -> None:
    ...
    for _ in data:
        # Type is already guaranteed by the list[int | float] type annotation
        pass
...
def main() -> None:
    ...
    _ = input_group.add_argument(
        "array", nargs="?", type=str, help='Array to sort (e.g., "1, 2, 3")'
    )
    _ = input_group.add_argument("-f", "--file", type=str, help="Read from file")
    _ = input_group.add_argument(
    ...
    args: ArgparseArgs = parser.parse_args()
