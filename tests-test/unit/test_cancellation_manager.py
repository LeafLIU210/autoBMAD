...
@pytest.mark.anyio
class TestTrackSDKExecution:
    """测试 track_sdk_execution 异步上下文管理器"""
    ...
    async def test_track_sdk_execution_registers_call(self, manager):
        """测试 track_sdk_execution 注册调用"""
        call_id = "context-test-call"

        async with manager.track_sdk_execution(call_id, "TestAgent"):
            # 在上下文内部，调用应该已注册
            assert call_id in manager._active_calls
            call_info = manager._active_calls[call_id]
            assert call_info.agent_name == "TestAgent"

    async def test_track_sdk_execution_marks_cleanup_on_exit(self, manager):
        """测试 track_sdk_execution 退出时标记清理完成"""
        call_id = "cleanup-test-call"

        async with manager.track_sdk_execution(call_id, "TestAgent"):
            # 在上下文内部，cleanup_completed 应该为 False
            call_info = manager._active_calls[call_id]
            assert call_info.cleanup_completed is False

        # 退出后，cleanup_completed 应该为 True
        call_info = manager._active_calls[call_id]
        assert call_info.cleanup_completed is True

    async def test_track_sdk_execution_marks_cleanup_on_exception(self, manager):
        """测试 track_sdk_execution 在异常时标记清理完成"""
        call_id = "exception-test-call"

        with pytest.raises(ValueError):
            async with manager.track_sdk_execution(call_id, "TestAgent"):
                raise ValueError("Test exception")

        # 即使发生异常，cleanup_completed 也应该为 True
        call_info = manager._active_calls[call_id]
        assert call_info.cleanup_completed is True

    async def test_track_sdk_execution_with_operation_name(self, manager):
        """测试 track_sdk_execution 带操作名称"""
        call_id = "operation-test-call"

        async with manager.track_sdk_execution(
            call_id, "TestAgent", "test_operation"
        ):
            call_info = manager._active_calls[call_id]
            assert call_info is not None

    async def test_track_sdk_execution_full_lifecycle(self, manager):
        """测试 track_sdk_execution 完整生命周期"""
        call_id = "lifecycle-context-call"
        ...
        # 4. 退出上下文后，cleanup_completed 应该为 True
        call_info = manager._active_calls[call_id]
        assert call_info.cleanup_completed is True
...
class TestMonitoringSingleton:
    """测试 monitoring 模块的单例模式"""

    def test_get_cancellation_manager_returns_same_instance(self):
        """测试 get_cancellation_manager 返回同一实例"""
    ...
    def test_reset_cancellation_manager_creates_new_instance(self):
        """测试 reset_cancellation_manager 创建新实例"""
        from autoBMAD.epic_automation.monitoring import (
            get_cancellation_manager,
            reset_cancellation_manager
        )
    ...
    def test_singleton_preserves_state(self):
        ...
        # 使用第一个引用注册调用
        manager1 = get_cancellation_manager()
        manager1.register_call("singleton-test-call", "TestAgent")

        # 获取第二个引用，应该能看到同样的调用
        manager2 = get_cancellation_manager()
        assert "singleton-test-call" in manager2._active_calls
