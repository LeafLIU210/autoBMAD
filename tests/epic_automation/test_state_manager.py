...
class TestStateManager:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_epic_stories(self, state_manager):
        """验证当前 Epic Story 旧记录被清理"""
        # 准备：插入历史记录
        await state_manager.update_story_status(
            '/epic-1/story-1.1.md', 'failed', epic_path='epic-1'
        )
        await state_manager.update_story_status(
            '/epic-1/story-1.2.md', 'completed', epic_path='epic-1'
        )
        await state_manager.update_story_status(
            '/epic-2/story-2.1.md', 'in_progress', epic_path='epic-2'
        )

        # 执行清理（只清理 epic-1 的 1.1 和 1.2）
        deleted = await state_manager.cleanup_epic_stories('epic-1', ['1.1', '1.2'])

        # 断言
        assert deleted == 2

        # 验证：epic-1 的记录被删除
        state1 = await state_manager.get_story_status('/epic-1/story-1.1.md')
        assert state1 is None
        state2 = await state_manager.get_story_status('/epic-1/story-1.2.md')
        assert state2 is None

        # 验证：epic-2 的记录仍然存在
        state3 = await state_manager.get_story_status('/epic-2/story-2.1.md')
        assert state3 is not None

    @pytest.mark.asyncio
    async def test_cleanup_temp_stories(self, state_manager):
        """验证 TEMP 路径记录被清理"""
        # 准备
        await state_manager.update_story_status(
            '/path/to/docs/stories/1.1.md',
            'pending',
            epic_path='epic-1'
        )
        await state_manager.update_story_status(
            'C:\\Users\\Admin\\AppData\\Local\\Temp\\tmp123\\test_story.md',
            'pending',
            epic_path='epic-test'
        )

        # 执行清理
        deleted = await state_manager.cleanup_temp_stories()

        # 断言
        assert deleted == 1

        # 验证：正常路径保留
        state1 = await state_manager.get_story_status('/path/to/docs/stories/1.1.md')
        assert state1 is not None
        # 验证：TEMP 路径删除
        state2 = await state_manager.get_story_status('C:\\Users\\Admin\\AppData\\Local\\Temp\\tmp123\\test_story.md')
        assert state2 is None

    @pytest.mark.asyncio
    async def test_cleanup_test_stories(self, state_manager):
        """验证测试标记记录被清理"""
        # 准备
        await state_manager.update_story_status(
            '/docs/stories/1.1.md',
        ...
        # 执行清理
        deleted = await state_manager.cleanup_test_stories()

        # 断言
        assert deleted == 2

        # 验证：正常 Story 保留
        state1 = await state_manager.get_story_status('/docs/stories/1.1.md')
        assert state1 is not None
        # 验证：测试 Story 删除
        state2 = await state_manager.get_story_status('/docs/stories/test_story_1.md')
        assert state2 is None
        state3 = await state_manager.get_story_status('/docs/stories/1.1-test.md')
        assert state3 is None

    @pytest.mark.asyncio
    async def test_initialize_for_epic(self, state_manager):
        """验证统一初始化方法"""
        # 准备：混合数据
        await state_manager.update_story_status(
            '/epic-1/story-1.1.md',
        ...
        # 执行初始化
        stats = await state_manager.initialize_for_epic('epic-1', ['1.1', '1.2'])

        # 断言清理统计
        assert 'epic_stories' in stats
        assert 'temp_stories' in stats
        assert 'test_stories' in stats
        ...
        state2 = await state_manager.get_story_status('/epic-2/story-2.1.md')
