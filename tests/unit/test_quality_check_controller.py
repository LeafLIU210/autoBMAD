...
class TestQualityCheckController:
    ...
    async def test_run_check_phase_error(self, controller):
        """测试检查阶段出错的情况"""
        # 模拟 Agent 的 execute 方法抛出异常
        # 这样 _run_check_phase 中的 try-except 就会捕获异常并返回空 dict
        controller.agent.execute = AsyncMock(side_effect=Exception("Agent execute failed"))

        # 模拟日志记录
        with patch.object(controller.logger, 'error'):
            result = await controller.run()

        # 当检查阶段出错时，run() 方法会捕获异常并返回空结果
        assert result["status"] == "completed"
        assert result["initial_error_files"] == []
        assert result["final_error_files"] == []
        assert result["sdk_fix_attempted"] == False
    ...
    async def test_execute_sdk_fix_success(self, controller):
        ...
        # 模拟 execute_sdk_call 返回成功结果
        # 注意：在被导入的模块中打补丁
        with patch("autoBMAD.epic_automation.agents.sdk_helper.execute_sdk_call", new_callable=AsyncMock) as mock_execute:
    ...
    async def test_execute_sdk_fix_failure(self, controller):
        ...
        mock_result = SDKResult(
            ...
            errors=["SDK internal error"]
        )

        # 模拟 execute_sdk_call 返回失败结果
        # 注意：在被导入的模块中打补丁
        with patch("autoBMAD.epic_automation.agents.sdk_helper.execute_sdk_call", new_callable=AsyncMock) as mock_execute:
            mock_execute.return_value = mock_result
