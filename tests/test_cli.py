...
class TestFormatOutput:
    ...
    def test_format_output_json(self):
        ...
        output = format_output([1, 2, 3], [1, 2, 3], 'json')
        import json
        parsed = json.loads(output)
        assert parsed == {"input": [1, 2, 3], "sorted": [1, 2, 3]}

    def test_format_output_default(self):
        """Test default output formatting."""
        output = format_output([3, 2, 1], [1, 2, 3], 'default')
        assert '[1, 2, 3]' in output

    def test_format_output_detailed(self):
        """Test detailed output formatting."""
        output = format_output([1, 2, 3], [1, 2, 3], 'detailed')
        assert 'Input:' in output and 'Sorted:' in output
